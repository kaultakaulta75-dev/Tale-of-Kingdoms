        ...
        api.getConquestInstanceStorage().mostRecentInstance().ifPresent(instance -> {
            if (instance.hasAttacked(player.getUuid())) {
                if (player.getMainHandStack().getItem() == Items.WOODEN_SWORD) {
                    ...
                    Translations.GUILDMEMBER_START_FIGHT.send(player);
                    final int[] countdown = {3};
                    api.getScheduler().repeatN(server -> {
                        if (player.isAlive()) player.sendMessage(Text.literal("" + countdown[0]), false);
                        countdown[0] = countdown[0] - 1;
                    }, 3, 0, 20);
                    api.getScheduler().queue(server -> {
                        final ImprovedFollowTargetGoal<PlayerEntity> goal = new ImprovedFollowTargetGoal<>(
                                this, EntityType.PLAYER, 0, true, false,
                                target -> target.getUuid().equals(player.getUuid())
                        );
                        ...
                        api.getScheduler().queue(server2 -> {
                            ...
                            if (guildPlayer != null && player instanceof ServerPlayerEntity serverPlayer) {
                                GuildQuestProgression.rewardWorthiness(instance, serverPlayer, 2);
                            }
                            this.setStackInHand(Hand.MAIN_HAND, new ItemStack(Items.IRON_SWORD));
                            sparring = false;
                        }, 160);
